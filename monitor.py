...
class MarketFeed:
    ...
    def __init__(self):
        ...
        self.last_market_check_fail_ms = 0  # 市场未找到时的冷却时间戳
    ...
    def resolve_round(self):
        ...
        # 市场未找到时的冷却：避免过密轮询 gamma-api
        now = now_ms()
        if self.last_market_check_fail_ms and now - self.last_market_check_fail_ms < 2000:
            return False
        ...
        try:
            ...
            if not events:
                ...
                self.last_market_check_fail_ms = now
                return False
            m = events[0]["markets"][0]
            ids = json.loads(m["clobTokenIds"])
            # 校验 UP/DOWN 顺序：优先信任 outcomes 字段
            outcomes = json.loads(m.get("outcomes", "[]"))
            if outcomes and len(outcomes) >= 2 and outcomes[0].lower() != "up":
                self.up_token, self.down_token = ids[1], ids[0]
                log.d(f"检测到非标准顺序，已交换 UP/DOWN token")
            else:
    ...
    def fetch_book(self, token_id):
        ...
        try:
            ...
            bids.sort(key=lambda x: x[0], reverse=True)  # 降序：[0] = best(最高) bid
            return asks, bids
        except Exception:
            return [], []

    def tick_books(self):
        self.up_asks, self.up_bids = self.fetch_book(self.up_token)
        self.down_asks, self.down_bids = self.fetch_book(self.down_token)
    ...
    def best_bid(self, side):
        ...
        arr = self.up_bids if side == "UP" else self.down_bids
...
class PaperTrader:
    ...
    def _exit(self, reason):
        if not self.leg1:
            return
        # 退出应该用 bid 价格（卖出给买方），而不是 ask 价格
        exit_price = self.feed.best_bid(self.leg1["side"])
        if exit_price is None or exit_price >= 1:  # 盘口空 → 假设平价退出（保守取 0 损益）
